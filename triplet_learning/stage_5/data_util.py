...
def load_data(Data_file,n_labels,IMG_HEIGHT=299,IMG_WEIGHT=299,BATCH_SIZE=3):
    ...
    for epoch in itertools.count():
        ...
        for batch_index in range(BATCH_SIZE/3):
            ...
            data_input.append(data_anchor)
            data_input.append(data_positive)
            data_input.append(data_negative)
            label_input.append(label_anchor)
            label_input.append(label_positive)
            label_input.append(label_negative)


        yield {"image_data": data_input,\
                "image_label": label_input}
