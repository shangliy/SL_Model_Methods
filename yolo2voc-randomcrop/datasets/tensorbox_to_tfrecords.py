...
def _process_image(directory, name, rects):
    ...
    for rect in rects:
        ...
        bboxes.append((float(rect["y1"]) / shape[0],
                       float(rect["x1"]) / shape[1],
                       float(rect["y2"]) / shape[0],
                       float(rect["x2"]) / shape[1]
                       ))
        if (rect["y1"] >rect["y2"] ) or (rect["x1"] >rect["x2"] ):
            print filename
...
def _add_to_tfrecord(dataset_dir, name, rect, tfrecord_writer):
    ...
    image_data, shape, bboxes, labels, labels_text, difficult, truncated = \
        _process_image(dataset_dir, name, rect)
...
def run(data_json,dataset_dir, output_dir, name='voc_train', shuffling=True):
    ...
    filenames = []
    rects = {}
    for json_img in json_data_set:
        file_name = json_img["image_path"]
        img_name = file_name[file_name.rfind('/')+1:]
        filenames.append(file_name)
        rects[img_name[:-4]] = json_img["rects"]
    ...
    while i < len(filenames):
        ...
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:
            ...
            while i < len(filenames) and j < SAMPLES_PER_FILES:
                ...
                filerect = rects[img_name[:-4]]
                _add_to_tfrecord(dataset_dir, filename,filerect,tfrecord_writer)
                i += 1
                j += 1
            fidx += 1
